...
const useToken = ({ provider }) => {
  ...
  const feeFunctions = async (contractAddress: string) => {
    ...
    try {
      ...
      if (byteCode.includes(ethers.utils.id("service_fee()").slice(2, 10))) {
        ...
        byteCode.includes(ethers.utils.id("fee_data()").slice(2, 10))
      ) {
      ...
      if (!isSameContract(token.address)) {
        ...
        if (compare(allowance.toString(), amount.raw) === -1) {
          ...
          if (compare(allowance, 0) === 1) {
            // Approve as 0
            let approveEstimateGas = await tokenContract.estimateGas.approve(
              spender,
              0
            );
            approveEstimateGas = approveEstimateGas.add(
              approveEstimateGas.div(GAS_MULTIPLIER)
            );
            ...
            const { hash: approveHash } = await tokenContract.approve(
              spender,
              0,
              {
          ...
          onWait({
            ...
            txText: `Approve ${parseFloat(Number(amount.format).toFixed(8))} ${
              token.symbol
            }`,
            txHash: "Approve this transaction on your EVM wallet",
          });
  ...
  const fetchTokenPrice = async (
    ...
    const priceData = await tokenPrice({
      network: chainId,
      token:
        tokenAddress === VETH_ADDRESS ? Addresses[chainId].weth : tokenAddress,
    }).unwrap();
  ...
  const slippageFromPrice = async (
    ...
    try {
      ...
      return Number(tokenPrice) > 100000
        ? 0.1
        : Number(tokenPrice) > 10000
        ? 0.5
        : 1;
    } catch (error) {
      console.error(error);
