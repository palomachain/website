...
import { SupportSystems, UpgradeMigrationGuide } from 'utils/data';
...
const Migration = () => {
  ...
  const onChangeStep = (step: number) => {
    ...
    if (nextStep > UpgradeMigrationGuide[currentTab].length - 1) {
  ...
  const myStep = useMemo(() => {
    return UpgradeMigrationGuide[currentTab][currentStep];
  ...
  return (
    <section className={style.container}>
      <h1>Paloma LightNode Migration Guide</h1>
      <p className={style.describe}>Choose your OS and follow the instructions below.</p>
      <div className={style.tabs}>
        {Object.values(SupportSystems).map(
          (system, index) =>
            system !== SupportSystems.VPS && (
              <div
                key={index}
